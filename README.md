...
打开工程后运行 `CodeBuddyRemote` target。推荐使用右上角菜单或连接设置里的 `扫码绑定` 扫描 Mac 端 `codebuddy-remote` 打印的二维码。局域网模式下，也可以手动输入 Mac 端打印的局域网 URL 和 token；Relay 模式下，也可以手动输入 Relay URL、配对码和配对密钥。App 会订阅事件流、显示终端输出，并把手机输入发送到同一个长期驻留的 CodeBuddy CLI session。
